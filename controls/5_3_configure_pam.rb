...
control 'cis-dil-benchmark-5.3.4' do
  ...
  describe.one do
    %w(common-password system-auth password-auth).each do |f|
      describe file("/etc/pam.d/#{f}") do
        its(:content) { should match(/^password\s+(sufficient|required)\s+pam_unix\.so\s+(\S+\s+)*sha512$/) } # '\s+' after 'password' fixes the regex to account for extra whitespaces introduced by Ansible's PAM module.
      end
    end
  end
end
